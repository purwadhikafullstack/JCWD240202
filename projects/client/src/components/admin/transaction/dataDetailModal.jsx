                ...
                if (value?.cart_id === props?.detailId) {
                    ...
                    return (
                        <div key={index} className="flex-auto w-full">
                            ...
                            <div className={`border truncate w-fit rounded-full px-3
                                ${
                            ...
                            <hr/>
                                <div className="flex justify-between mt-3">
                                    <div className="text-slate-500">No. Invoice</div>
                                    <div className="text-[#34498d]">{value?.invoice_number }</div>
                                </div>
                            ...
                            : null}
                                ...
                                <CardDetail data={value?.cart?.cart_products} func={{openMore, setOpenMore}} />
                                </div>
                                {value?.cart?.cart_products?.length > 1 ?
                                    <div className=" flex justify-end">
                                        {!openMore ? <div onClick={()=>setOpenMore(!openMore)} className="flex items-center mb-3 cursor-pointer w-fit text-sm text-[#2296f3]">+{value?.cart?.cart_products?.length - 1} Other Products
                                            <IoIosArrowDown className="text-[20px] ml-2" />
                                        </div> : <div onClick={()=>setOpenMore(!openMore)} className="flex items-center mb-3 cursor-pointer w-fit text-[#2296f3]"> Show Less
                                ...
                                <div className="mt-2 border-b border-t py-3">
                                    ...
                                    <div className="flex mt-3">
                                        <div className="w-28 text-slate-500">Courier</div>
                                        <div> <span className="mr-4">:</span>{value?.courier} - { value?.shipping_method}</div>
                                    </div>
                                    <div className="flex">
                                    ...
                                    <div className="flex">
                                        ...
                                        <div>
                                            <div className="font-semibold"><span className="mr-4 font-normal">:</span>{value?.receiver_name}</div>
                                            <div className="ml-5">{value?.receiver_number}</div>
                                            <div className="ml-5">{value?.shipping_address }</div>
                                        </div>
                                    </div>
                            </div>
                            <div className="mt-4 ">
                                ...
                                <div className="flex justify-between my-1">
                                    <div className="text-slate-500">Total Price ({value?.cart?.cart_products?.length} products)</div>
                                    <div>Rp. {value?.total_cart_price.toLocaleString('id')}</div>
                                </div>
                                <div className="flex justify-between">
                                    <div className="text-slate-500">Total Shipping Fee ({value?.total_weight} gr)</div>
                                    <div>Rp. {value?.shipping_fee.toLocaleString('id')}</div>
                                </div>
                                <hr/>
                                <div className="flex justify-between mt-2">
                                    <div className="font-semibold">Total</div>
                                    <div className="font-semibold">Rp. {value?.total.toLocaleString('id')}</div>
                                </div>
                            </div>
